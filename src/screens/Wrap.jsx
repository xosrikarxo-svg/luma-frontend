  ...
  return (
    <div style={{ minHeight:'100vh', background:BG, color:TEXT, fontFamily:'system-ui,-apple-system,sans-serif', display:'flex', flexDirection:'column', justifyContent:'space-between', maxWidth:480, margin:'0 auto', padding:'0 24px' }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:12, paddingBottom:48 }}>
        <button onClick={onRestart} style={{ width:'100%', height:52, borderRadius:9999, background:ACCENT, border:'none', color:BG, fontSize:15, fontWeight:700, cursor:'pointer', letterSpacing:'-0.2px' }}>
        ...
        {onReconnect && reconnectStatus !== 'expired' && (
          <button
            ...
            style={{
              ...
              border:`1.5px solid ${reconnectStatus === 'waiting' ? 'rgba(244,162,97,0.4)' : ACCENT}`,
              color: reconnectStatus === 'waiting' ? 'rgba(244,162,97,0.5)' : ACCENT,
              fontSize:15, fontWeight:600, cursor: reconnectStatus === 'waiting' ? 'default' : 'pointer',
              fontFamily:'inherit'
            }}
          >
